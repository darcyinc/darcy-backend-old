...
import type { CreateUserDto } from './createUser';

// Returns the user token if authentication is successful
export default async function authenticateUser({ email, password }: CreateUserDto) {
	...
	return existingUser.token as string;
}
